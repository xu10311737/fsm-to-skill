# fsm-to-skill · 将复杂流程转换成状态机控制的 SKILL

> 让大模型回归“推理”本职，将“流程控制”交还给经典而严谨的有限状态机（FSM）。

> 基于状态机的完全本地化、轻量级 的 SKILL 编排与生成工具。 画布建流程，本地可调式，一键导出技能。
...
- 状态机作为唯一的流程控制器，流程完全由状态机控制。
- 唯一入口：状态机作为 Agent 交互的唯一入口，输出每一步（Step）的标准 SOP 引导和专属 Prompt。
- 按需呈现：Agent 每一步只看该看的内容，降低认知负载，彻底避免遗忘关键约束。
- 输入校验：每步自定义对 Agent 的输出进行严格校验，并将校验结果循环返回给 Agent 纠错。
...
| 节点 | 说明 |
| --- | --- |
| **start** | 每个工作流DAG 入口，声明初始变量。 |
| **code** | 子进程执行 Python。输入为 argparse 风格参数 schema，返回字典自动成为输出变量，语法错误自动检查、 单节点调试。 支持错误处理: 重试/error分支 |
| **llm** | 渲染 Prompt，支持插入变量。引擎内不调用模型，作为 Agent 出口返回消息。 |
| **if** | 条件分支，支持1个或多个IF出口。 |
| **for** | 对 list 变量逐项迭代，子画布编排循环体。 |
...
- **实时状态**：节点状态实时追踪: 各节点耗时、状态根据运行实时显示。
